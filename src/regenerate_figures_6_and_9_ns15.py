# ============================================================
# regenerate_figures_6_and_9_ns15.py
#
# Purpose
#   Regenerate manuscript Figures 6 and 9 from one internally consistent
#   centerline-density POD calculation using all 15 clean pressure snapshots.
#
# Reads Tecplot ASCII files from the repository's data/ folder:
#   P=15.dat, P=16.dat, P=18.dat, P=19.dat, P=20.dat,
#   P=22.dat, P=23.dat, P=24.dat, P=25.dat, P=26.dat,
#   P=27.dat, P=28.dat, P=29.dat, P=30.dat, P=33.dat
#
# Outputs are written only to:
#   generated/Figures_6_9_Ns15/
#
# The script stops if even one required snapshot is missing. This guard prevents
# a five-snapshot figure from being labeled as an N_s=15 calculation.
#
...
# Uniformly parsed main-zone snapshot set stated in the manuscript (N_s=15).
SELECTED_PRESSURES = [
...
def gradient_moment_width(x, g, xs, window_factor=8.0, fallback_width=None):
    ...
    if np.sum(mask) < 5 or np.nansum(w[mask]) <= 0:
        return np.nan
    num = np.trapezoid(((x[mask] - xs) ** 2) * w[mask], x[mask])
    den = np.trapezoid(w[mask], x[mask])
    if den <= 0:
        return np.nan
...
def write_centerline_full_pod_spectrum_table(cases):
    ...
    plt.close(fig)
    shutil.copy2(
        OUTDIR / "Fig_PRF_V7_centerline_POD_full_spectrum_Rho.png",
        OUTDIR / "Fig6_centerline_POD_full_spectrum_Rho.png",
    )
    shutil.copy2(
        OUTDIR / "Fig_PRF_V7_centerline_POD_full_spectrum_Rho.pdf",
        OUTDIR / "Fig6_centerline_POD_full_spectrum_Rho.pdf",
    )
...
def write_figure_9_from_same_pod(pod_by_coord):
    """Write Figure 9 from the exact xi_jump POD already used in Figure 6."""
    pod = pod_by_coord["xi_jump"]
    grid = pod["grid"]
    mean = pod["mean"]
    modes = pod["VT"]
    ...
    for k in range(min(3, modes.shape[0])):
        ax.plot(
            grid,
            modes[k, :],
            lw=2.0,
            label=f"Mode {k + 1}, energy={energy_frac[k] * 100.0:.2f}%",
        )

    ax.axvline(0.0, color="k", lw=1.2, ls="--")
    ax.set_xlim(GRID_CONFIG["xi_jump"]["min"], GRID_CONFIG["xi_jump"]["max"])
    ax.set_xlabel(r"$\xi_j=(x-x_s)/\delta_j$", fontweight="bold")
    ax.set_ylabel("POD mode / mean", fontweight="bold")
    ax.set_title("POD modes of shock-centered density profiles", fontweight="bold")
    ax.legend(loc="upper right", frameon=True, fontsize=9)
    ax.grid(True, alpha=0.3)

    # Current paper name, descriptive name, and legacy source name are all
    # rendered from the same Matplotlib figure object.
    for stem in ("pod_modes_rho", "Fig9_pod_modes_rho", "pod_modes_xi_jump_rho"):
        fig.savefig(OUTDIR / f"{stem}.png", dpi=DPI, bbox_inches="tight")
        fig.savefig(OUTDIR / f"{stem}.pdf", bbox_inches="tight")
    plt.close(fig)

    audit_path = OUTDIR / "Fig6_Fig9_density_POD_modes.csv"
    with open(audit_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["mode", "individual_energy_percent", "cumulative_energy_percent"])
        for k, (e, c) in enumerate(zip(energy_frac, cumulative), start=1):
            writer.writerow([k, f"{100.0 * e:.10f}", f"{100.0 * c:.10f}"])

    print("[OK] wrote pod_modes_rho.pdf/.png and aliases")
    print(f"[OK] wrote {audit_path.name}")
    return {
        "E1_percent": float(100.0 * energy_frac[0]),
        "E1_plus_E2_percent": float(100.0 * cumulative[1]),
        "E1_plus_E2_plus_E3_percent": float(100.0 * cumulative[2]),
    }
...
def main():
    ...
    by_pressure = {get_pressure_label(f): f for f in files}
    missing = [p for p in SELECTED_PRESSURES if p not in by_pressure]
    if missing:
        missing_names = ", ".join(f"P={p}.dat" for p in missing)
        raise FileNotFoundError(
            "Cannot produce an N_s=15 figure because required clean snapshots "
            f"are missing: {missing_names}"
        )
    selected_files = [by_pressure[p] for p in SELECTED_PRESSURES]
    ...
    spectrum_rows, pod_by_coord = write_centerline_full_pod_spectrum_table(cases)
    figure_9 = write_figure_9_from_same_pod(pod_by_coord)

    summary_path = OUTDIR / "Figures_6_9_Ns15_run_summary.txt"
    with open(summary_path, "w") as f:
        f.write("Figures 6 and 9: internally consistent N_s=15 regeneration\n")
        f.write("==========================================================\n\n")
        f.write(f"N_s = {len(selected_files)}\n")
        ...
        f.write("\nFigure 9 uses the same xi_jump POD object as Figure 6:\n")
        f.write(f"  E1={figure_9['E1_percent']:.10f}%\n")
        f.write(f"  E1:2={figure_9['E1_plus_E2_percent']:.10f}%\n")
        f.write(f"  E1:3={figure_9['E1_plus_E2_plus_E3_percent']:.10f}%\n")

    print(f"\n[OK] wrote {summary_path.name}")
    print("\nDone. Figures 6 and 9 plus numerical audit files are in:")
    print(OUTDIR)
