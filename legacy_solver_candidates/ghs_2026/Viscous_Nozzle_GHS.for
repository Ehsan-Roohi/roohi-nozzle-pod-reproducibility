...
C     Read InputData first so new GHS parameters are available on restart.
      CALL DATA2
...
*--set the y coordinates
          ...
          DO 420 L=1,MNSG
            DO 410 K=1,MNSG
              CCG(2,M,L,K)=RF(0)
              IF (IMOLMODEL.EQ.2) THEN
                VRR0=4.D0*BOLTZ*FTMP/SPM(5,1,1)
                CALL CALC_GHS_CVR(VRR0,SPM(5,1,1),CVR0)
                CCG(1,M,L,K)=10.D0*CVR0
              ELSE
...
C     mfp/Kn are computed after TEMP is available
...
*--TROT is the rotational temperature, see eqn (11.11)
      ...
      IF (IMOLMODEL.EQ.2) THEN
        VRRTH=4.D0*BOLTZ*TEMP/SPM(5,1,1)
        CALL CALC_GHS_SIG(VRRTH,SPM(5,1,1),SIGT_GHS)
        mfp=1.D0/(SQRT(2.D0)*SIGT_GHS*DENN)
      ELSE
...
!======================================================================
      SUBROUTINE CALC_GHS_SIG(VRR,RMASS,SIGT)
*
*--GHS total collision cross-section for N2.
*--Hash, Moss and Hassan parameters are read from InputData.txt.
*--VRR is the square of the relative speed, RMASS is the reduced mass.
*
      Include 'common.txt'
      REAL VRR,RMASS,SIGT
      DOUBLE PRECISION DVRR,DRMASS,DSIGT,ETSTAR

      DVRR=DBLE(VRR)
      DRMASS=DBLE(RMASS)
      IF (DVRR.LT.1.D-30) DVRR=1.D-30

*--reduced collision energy, E_t/epsilon
      ETSTAR=0.5D0*DRMASS*DVRR/(BOLTZ*GHS_EPSK)
      IF (ETSTAR.LT.1.D-12) ETSTAR=1.D-12

*--sigma_T = sigma^2 [a1 (E/eps)^(-w1) + a2 (E/eps)^(-w2)]
      DSIGT=GHS_SIG*GHS_SIG*
     &     (GHS_A1*ETSTAR**(-GHS_W1)
     &     +GHS_A2*ETSTAR**(-GHS_W2))
      SIGT=SNGL(DSIGT)

      RETURN
      END
!======================================================================
      SUBROUTINE CALC_GHS_CVR(VRR,RMASS,CVROUT)
*
*--Product of relative speed and GHS total cross-section, g*sigma_T(g).
*
      REAL VRR,RMASS,CVROUT,SIGT,VRRS

      VRRS=VRR
      IF (VRRS.LT.1.E-30) VRRS=1.E-30

      CALL CALC_GHS_SIG(VRRS,RMASS,SIGT)
      CVROUT=SQRT(VRRS)*SIGT
...
*--VR is the relative speed
      ...
      MS=IPS(M)
      IF (IMOLMODEL.EQ.2) THEN
        CALL CALC_GHS_CVR(VRR,SPM(5,LS,MS),CVR_GHS)
        CVR=CVR_GHS
      ELSE
        CVR=VR*SPM(1,LS,MS)*
     &      ((2.*BOLTZ*SPM(2,LS,MS)/(SPM(5,LS,MS)*VRR))
     &      **(SPM(3,LS,MS)-0.5))/SPM(6,LS,MS)
      ENDIF
*--for GHS, CVR is based on g*sigma_T(g); otherwise eqn (4.63)
      RETURN
      END
!======================================================================
      SUBROUTINE ELASTIC
...
*--VCCM defines the components of the centre-of-mass velocity, eqn (2.1)
      IF (IMOLMODEL.EQ.2) THEN
*--GHS uses hard-sphere angular scattering; only sigma_T(g) changes
        B=2.*RF(0)-1.
        A=SQRT(1.-B*B)
        VRCP(1)=B*VR
        C=2.*PI*RF(0)
        VRCP(2)=A*COS(C)*VR
        VRCP(3)=A*SIN(C)*VR
      ELSEIF (ABS(SPM(4,LS,MS)-1.).LT.1.E-3) THEN
*--use the VHS logic
        B=2.*RF(0)-1.
     ...
     &	Erot_in,Etr_out,Erot_out,Vx'
		...
		DO i=1,NCX
			...
			IF ((ITypeQw==1).and.(i.GT.INZ(2))) THEN
				Tp=Tw(i)
				DTw=Tw(i)*QwRF*(QQ-DesQw(i))/(ABS(DesQw(i))+QwEPS)
				Tw(i)=Tw(i)+DTw
			Else
				Tp=Tw(i)
			end if 
...
*--the simulated region is from x=XB(1) to x=XB(2)
	...
	READ(221,*)	TSURF(3)
      ...
      READ(221,*)	SP(4,1)
      READ(221,*)	SP(5,1)
C     Molecular model switch and GHS parameters.
C     IMOLMODEL=1 keeps the original VHS/VSS model.
C     IMOLMODEL=2 uses GHS total cross-section for N2.
      READ(221,*)	IMOLMODEL
      IF (IMOLMODEL.EQ.2) THEN
        READ(221,*) GHS_SIG
        READ(221,*) GHS_EPSK
